...
(function () {
    ...
    function AddContact(fullName, contactNumber, emailAddress) {
        ...
        if (newContact.serialize()) {
            ...
            if (value != "") {
                ...
                (console.log("FAILED TO SERIALIZE CONTACT DATA"));
        }
    }
    ...
    function ValidateContactPage(errorMessages) {
        ValidateElementInput("full-name", regExpPatterns.fullNamePattern, errorMessages, "Invalid Name");
        ValidateElementInput("email-address", regExpPatterns.emailAddressPattern, errorMessages, "Invalid Email");
        ValidateElementInput("contact-number", regExpPatterns.contactNumberPattern, errorMessages, "Invalid Contact Number");
    }
    function ValidateElementInput(elementId, expression, errorElement = null, errorMessage = "") {
            ...
            default:
                {
                    ...
                    $("#edit-button").on("click", function (event) {
                        ...
                        if (confirm(`Are you sure you want to make these changes to ${editContact.FullName} ?`)) {
                            editContact.FullName = fullName.value;
                            editContact.EmailAddress = emailAddress.value;
                            editContact.ContactNumber = contactNumber.value;
                ...
                for (const user of JSON.parse(userData).users) {
                    if (user.Username === usernameInput.value &&
                        ...
                        if (typeof foundUser.serialize() === "string") {
                            sessionStorage.setItem("user", foundUser.serialize());
                            successFlag = true;
                        }
                        break;
                    }
    ...
    function CheckLogin() {
        if (sessionStorage.getItem("user") != null) {
            let loginNavAnchor = document.querySelector("#login>a");
            let loginNavListItem = document.querySelector("#login>a");
    ...
    function Start() {
        ...
        fetchAndPromiseTestOne();
        fetchAndPromiseTestTwo();
    }
